...
> **本フォルダ（current/docs/domain/）が生きた正本**（2026-07-06 移管）。
> 出典（採用時スナップショット・凍結）: `../../../proposal/2026-04-30-zero-base-v4/`
> 本文中の `proposal/2026-04-30-zero-base-v4/...` 表記は採用時（2026-04-30）の史実。現在の実体はこのフォルダ。
> 07_review_prompts のみ `current/adapter/review_prompts/` が生きた正本（12値対応済み）。
> status 語彙は 2026-07-06 に 12 値へ拡張（正本: `../status_vocabulary.md`）。
...
- [ ] `proposal/2026-04-30-zero-base-v4/` 配下の全 11 ファイル + `../../adapter/review_prompts/` 配下の 7 ファイルが揃う
...
- [ ] `../../adapter/review_prompts/` 7 本がそれぞれ独立して LLM に貼り付けて使える状態
...
| `../../adapter/review_prompts/` | 7 本の intake-stage レビュープロンプト | **主要成果物 3** |
...
- 現行 `adapter/intake_adapter_prompt.md` は本提案の `../../adapter/review_prompts/intake-coverage-review.md` 等を参照する形に再構成される
