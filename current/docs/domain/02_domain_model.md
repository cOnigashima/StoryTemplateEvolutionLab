...
- Section 12. Status（Status meta + 12 値）
...
| Japanese | ステータス |
| Category | **Meta-Field** |
| Definition | The classification of how a kernel item, bible facet, design proposal, or state record is currently filled — using one of 12 canonical values (11 at v4 adoption; genre_not_applicable added 2026-07-06) that capture both presence and intent. |
...
| Examples | （以下の 12 値を参照） |
| Lives in | （概念のみ。各項目の status フィールドに格納） |
| Cousins | DoR, DoD |

### Status の 12 値（v4 採用時は 11 値。2026-07-06 に genre_not_applicable を追加）
| 値 | 意味 | DoR-A 通過 | 例 |
|---|---|---|---|
...
| `genre_not_applicable` | このジャンルに不要 | ✓ | "純文学寄りなのでチート体系 facet 不要"（2026-07-06 追加） |
