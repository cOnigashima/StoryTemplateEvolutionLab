...
`StoryTemplateEvolution/current/adapter/intake_adapter_prompt.md` は **生成 prompt**（raw → digest + update_proposal を作る）。本 7 本は **検証 prompt**（生成された artifact が正しいかチェックする）。両者は異なる役割で並走する。
